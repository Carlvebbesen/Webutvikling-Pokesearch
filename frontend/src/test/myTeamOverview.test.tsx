...
describe("Tests for myTeamOverviewComponent:", () => {
  test("Verify if myTeamOverviewComponent renders correctly without pokemon in team", async () => {
    const doc = render(
      <MockedProvider>
        <RecoilRoot>
  ...
  test("Verify if myTeamOverviewComponent renders correctly with pokemon in team", async () => {
    const doc = render(
      <MockedProvider>
        <RecoilRoot>
  ...
  test("Verify if pokemon is removed when clicking remove from team", async () => {
    const doc = render(
      <MockedProvider>
        <RecoilRoot>
          <Team currentPokemon={testPokemon1} />
  ...
  test('Verify that "save team"-button is disabled when text field is empty', async () => {
    const doc = render(
      <MockedProvider>
        <RecoilRoot>
          <Team currentPokemon={testPokemon1} />
    ...
    const textFieldInput: Element | null = doc
  ...
  test('Verify that "save team"-button is working', async () => {
    const doc = render(
      <MockedProvider>
        <RecoilRoot>
          <Team currentPokemon={testPokemon1} />
...
describe("Tests for pokemonInTeamComponent:", () => {
  test("if pokemonInTeamComponent renders correctly", async () => {
    const doc = render(
      <MockedProvider>
        <RecoilRoot>
  ...
  test("Verify that remove-button is enabled", async () => {
    const doc = render(
      <MockedProvider>
        <RecoilRoot>
          <PokemonInTeamComponent pokemon={testPokemon1} />
