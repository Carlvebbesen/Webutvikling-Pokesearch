    ...
    return (
            ...
            <TableRow onClick={()=> setPopUpShow(pokemon.entry_number)} hover={true} key={pokemon.entry_number}>
                ...
                <TableCell padding={"none"} align="center" key={pokemon.entry_number+20000}>
                        <p data-cy="pokemon-name" style={{width: "100px"}}>{capitalize(pokemon.name)}</p>
                </TableCell>
                <TableCell data-cy="type-container" padding={"none"} align="center">
                    {pokemon.pokeTypes.map(type => <img data-cy={type} style={{marginRight: "10px"}}height="50" src={getPokeTypeIcon(type)} alt="PokeTypes"/>)}
                </TableCell>
                {Object.values(pokemon.stats).slice(1).map((value, index) => <TableCell data-cy={`stat_${index}`} className={style.statList} padding={"none"} align={"center"} key={index}>{value === null ? '-' : value}</TableCell>)}
                    </TableRow>
    )
};
