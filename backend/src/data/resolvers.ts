...
export const getResolvers = (
    pokemonModel: mongoose.Model<mongoose.Document<any, any, any>, any, any>,
    teamModel: mongoose.Model<mongoose.Document<any, any, any>, any, any>) => {
    const resolvers: IResolvers = {
        Query: {
            getFilteredPokemon:async (_, args: {input: FilterInput})=> {
                ...
                const searchCount = pokemonModel.countDocuments(query);
            ...
            getPokemonById:(_, args: {input: GetPokemonByIdInput}) => {
                return new Promise((resolve, reject) => {
                    const pokemon = pokemonModel.findOne( {entry_number: args.input.id}, function (err: Error, result: Pokemon | undefined) {
                        if (err || !result) {
        ...
        Mutation: {
            createTeam: async (_, args: {input: TeamInput}) => {
                let teamsWithProvidedName = await teamModel.countDocuments({ name: args.input.name });
                if (teamsWithProvidedName > 0) {
                    return new Promise((resolve, reject) => {
                        reject(new UserInputError("The team name specified is not unique."));
                    })
                }
                    const pokemons: Pokemon[] = await pokemonModel.find().where('entry_number').in(args.input.pokemons).exec();
                ...
                await pokemonModel.updateMany({entry_number: {$in: args.input.pokemons}}, {$inc:{"usage_count":1}});
            ...
            ratePokemon: async (_, args: {input: RatePokemonInput}) => {
                let pokemon = await pokemonModel.findOne({ entry_number: args.input.id });
